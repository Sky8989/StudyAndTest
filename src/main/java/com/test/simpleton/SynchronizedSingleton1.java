...
public class SynchronizedSingleton1 {
    ...
    private static SynchronizedSingleton1 singleton;
    ...
    /**
     * 双重校验锁 是对于加锁 的一种改进写法，通过使用同步代码块的方式减小了锁的范围。
    ...
    public static synchronized SynchronizedSingleton1 getInstance(){
        ...
        if(singleton == null){
            synchronized (SynchronizedSingleton1.class){
                if(singleton == null){
                    singleton = new SynchronizedSingleton1();
                }
            }
        }
...
/**
 * 但是，事情这的有这么容易吗？上面的代码看上去好像是没有任何问题。实现了惰性初始化，解决了同步问题，还减小了锁的范围，提高了效率。
 * 但是，该代码还存在隐患。隐患的原因主要和Java内存模型（JMM）有关。考虑下面的事件序列：
 *
 ...
 *
 * 在J2SE 1.4或更早的版本中使用双重检查锁有潜在的危险，
 * 有时会正常工作（区分正确实现和有小问题的实现是很困难的。取决于编译器，线程的调度和其他并发系统活动，不正确的实现双重检查锁导致的异常结果可能会间歇性出现。
 * 重现异常是十分困难的。） 在J2SE 5.0中，这一问题被修正了。volatile关键字保证多个线程可以正确处理单件实例
 */
